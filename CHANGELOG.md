...
The trade-off is the one flagged when the extension was first built: standing alone, it must **vendor** the engine. It keeps a committed copy of the four DOM-free files and refreshes them with its own `npm run sync-engine`, recording the source commit in `engine/VERSION`. Improvements here now reach it deliberately instead of automatically — a real cost, accepted because the extension has to be installable on its own.

`npm test` is back to this repo's own suite (**2299**); `npm run build:ext` and the `extension/engine` + `extension/icons` gitignore entries are removed. Everything the extension work produced *inside the engine* stays here and is still covered by these tests: the lowercase place-name detection, the two per-entity-toggle fixes, and the `FRIENDLY_LABELS` hoist into `recognizers.js`.
